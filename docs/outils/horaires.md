...
Prépare tes questions et leurs options (chaque option a un **emoji** — c'est via cet emoji que les gens votent en réagissant ; **clique sur l'emoji** pour en choisir un autre, ou tape un emoji / un nom d'emoji du serveur en bas du sélecteur). La poignée **⋮⋮** à côté de chaque option (et de chaque question) permet de la **glisser** pour changer l'ordre (ou ↑ ↓ au clavier une fois la poignée sélectionnée). Le bouton **＋ Nouvelle question** (en haut à droite) ajoute une question, la croix dans son en-tête la supprime (il en reste toujours au moins une). En Magna Arena, les options sont reconnues par leur intitulé (Installation, Rangement, TO / Régie), quel que soit leur ordre. Exemples typiques :
